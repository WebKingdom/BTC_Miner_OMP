...
#SBATCH --ntasks-per-node=18            # 64 processor core(s) per node 
#SBATCH --mem=16G                       # maximum memory per node
#SBATCH --partition=class-short         # class node(s)
#SBATCH --job-name="ssz_parallel3"
#SBATCH --output="hpc_parallel3-%j.out" # job standard output file (%j replaced by job id)
